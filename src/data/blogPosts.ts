...
// UPDATED: Get backend URL with new endpoint as default
function getBackendUrl(): string {
  // Updated to use the new API endpoint as fallback
  return process.env.NEXT_PUBLIC_BACKEND_API_URL || 'https://backend.mystocai.com';
}
...
export async function getAllBlogPosts(): Promise<BlogPost[]> {
  ...
  try {
    // UPDATED: Using the new endpoint structure
...
export async function getBlogPost(slug: string): Promise<BlogPost | undefined> {
  ...
  try {
    ...
    // UPDATED: Using the new endpoint
...
// UPDATED: Fetch current like count for a specific blog post using new endpoint
...
// UPDATED: Fetch current comment count for a specific blog post using new endpoint
