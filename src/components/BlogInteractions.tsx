...
// UPDATED: Define the different API endpoints
const BLOG_API_BASE = 'https://backend.mystocai.com';
const INTERACTION_API_BASE = 'https://stocai-backend-b0dsd8b2ctd4ebcn.centralindia-01.azurewebsites.net';

// UPDATED: Helper function to get the appropriate API URL based on operation type
function getApiUrl(operationType: 'blog' | 'interaction'): string {
  if (operationType === 'interaction') {
    // Use environment variable first, then fallback to the interaction endpoint
    return process.env.NEXT_PUBLIC_BACKEND_API_URL || INTERACTION_API_BASE;
  } else {
    // Use environment variable first, then fallback to the blog endpoint
    return process.env.NEXT_PUBLIC_BACKEND_API_URL || BLOG_API_BASE;
  }
}
...
export default function BlogInteractions({ blog, slug }: BlogInteractionsProps) {
  ...
  // UPDATED: Function to fetch current blog stats and comments using appropriate endpoints
  const fetchBlogData = async () => {
    const blogApiUrl = getApiUrl('blog');
    const interactionApiUrl = getApiUrl('interaction');

    try {
      // UPDATED: Fetch blog data from the main blog endpoint
      const blogResponse = await fetch(`${blogApiUrl}/blog/${blog.blog_id}`);
      
      // UPDATED: Fetch comments from the interaction endpoint
      const commentsResponse = await fetch(`${interactionApiUrl}/blogs/comment/${blog.blog_id}`);
  ...
  // UPDATED: Handle like using the interaction endpoint
  const handleLike = async () => {
    ...
    // UPDATED: Use the interaction API endpoint for like operations
    const interactionApiUrl = getApiUrl('interaction');

    try {
      const response = await fetch(
        `${interactionApiUrl}/blogs/${blog.blog_id}/like?user_id=${user.user_id}`,
        { method: 'POST' }
      );
  ...
  // UPDATED: Handle add comment using the interaction endpoint
  const handleAddComment = async (text?: string) => {
    ...
    // UPDATED: Use the interaction API endpoint for comment operations
    const interactionApiUrl = getApiUrl('interaction');
    ...
    try {
      const response = await fetch(
        `${interactionApiUrl}/blogs/${blog.blog_id}/comment`,
        {
          method: 'POST',
